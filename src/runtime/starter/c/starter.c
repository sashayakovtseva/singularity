...
static void prepare_scontainer_stage(int stage) {
    ...
    if (capget(&header, data) == -1) {
        singularity_message(ERROR, "Failed to get process capabilities\n");
        exit(1);
    }
    ...
    for (caps_index = 0; caps_index <= last_cap; caps_index++) {
        if (!(config.capBounding & (1ULL << caps_index))) {
            if (prctl(PR_CAPBSET_DROP, caps_index) == -1) {
    ...
    if ( !(config.nsFlags & CLONE_NEWUSER) ) {
        ...
        if ( uid == 0 ) {
            ...
            if (config.numGID >= 2) {
                ...
                if (setgroups(config.numGID-1, &config.targetGID[1]) == -1) {
            ...
            if (config.numGID >= 1) {
                ...
                if (setresgid(targetGID, targetGID, targetGID) == -1) {
            ...
            if (config.targetUID != 0) {
                ...
                if (setresuid(targetUID, targetUID, targetUID) == -1) {
...
static int enter_namespace(char *nspath, int nstype) {
    ...
    switch(nstype) {
    case CLONE_NEWPID:
        singularity_message(VERBOSE, "Entering pid namespace\n");
        ...
        break;
    case CLONE_NEWNET:
        singularity_message(VERBOSE, "Entering network namespace\n");
        ...
        break;
    case CLONE_NEWIPC:
        singularity_message(VERBOSE, "Entering ipc namespace\n");
        ...
        break;
    case CLONE_NEWNS:
        singularity_message(VERBOSE, "Entering mount namespace\n");
        ...
        break;
    case CLONE_NEWUTS:
        singularity_message(VERBOSE, "Entering uts namespace\n");
        ...
        break;
    case CLONE_NEWUSER:
        singularity_message(VERBOSE, "Entering user namespace\n");
...
#ifdef NS_CLONE_NEWCGROUP
    case CLONE_NEWCGROUP:
        singularity_message(VERBOSE, "Entering cgroup namespace\n");
        break;
#endif /* NS_CLONE_NEWCGROUP */
    default:
        singularity_message(VERBOSE, "Entering unknown namespace\n");
        errno = EINVAL;
        return(-1);
    }
...
static void setup_userns(const struct uidMapping *uidMapping, const struct gidMapping *gidMapping) {
    ...
    singularity_message(DEBUG, "Write 'deny' to setgroups file\n");
    ...
    if (map_fp != NULL) {
        ...
        singularity_message(ERROR, "Could not open setgroups for write: %s\n", strerror(errno));
        exit(1);
    }
...
static void list_fd(struct fdlist *fl) {
    ...
    fd_proc = open("/proc/self/fd", O_RDONLY);
    if (fd_proc  < 0) {
        singularity_message(ERROR, "Failed to open /proc/self/fd: %s\n", strerror(errno));
        exit(1);
    }

    dir = fdopendir(fd_proc);
    if (dir == NULL) {
...
__attribute__((constructor)) static void init(void) {
    ...
    if (stage_pid == 0) {
        ...
        // handle user input, read capabilities, check required namespaces
        if (config.isSuid) {
            priv_escalate();
    ...
    if (wait(&status) != stage_pid) {
        singularity_message(ERROR, "Couldn't wait child\n");
        exit(1);
    }
    ...
    if (config.isInstance) {
        ...
        if (forked == 0) {
            if (setsid() < 0) {
                singularity_message(ERROR, "Couldn't become session leader: %s\n", strerror(errno));
                exit(1);
            }
            umask(0);
        } else {
    ...
    if ((config.nsFlags & CLONE_NEWUSER) == 0 && get_nspath(user) == NULL) {
        ...
        if (get_nspath(user)) {
            if (enter_namespace(get_nspath(user), CLONE_NEWUSER) == -1) {
    ...
    // close unattended file descriptors opened during scontainer stage1
    // execution, that may not be accurate depending of fs operations done
    // in stage1, but should work for most engines.
    for (i = 0; i < fd_after.num; i++) {
        ...
        if (readlink(source, target, PATH_MAX) == -1 ) {
        ...
        if (fcntl(fd_after.fds[i], F_SETFD, FD_CLOEXEC) == -1) {
            singularity_message(DEBUG, "Couldn't set FD_CLOEXEC on file descriptor %d: %s", fd_after.fds[i], strerror(errno));
        }
    }
    free(source);
    free(target);
    ...
    if (get_nspath(mnt) == NULL) {
        ...
        if (create_namespace(CLONE_NEWNS) == -1) {
            singularity_message(ERROR, "Failed to create mount namespace: %s\n", strerror(errno));
            exit(1);
        }
        if (mount(NULL, "/", NULL, propagation, NULL) == -1) {
    ...
    if (socketpair(AF_UNIX, SOCK_STREAM|SOCK_CLOEXEC, 0, rpc_socket) == -1) {
        singularity_message(ERROR, "Failed to create communication socket: %s\n", strerror(errno));
        exit(1);
    }
    ...
    if (get_nspath(pid)) {
        if (enter_namespace(get_nspath(pid), CLONE_NEWPID) == -1) {
            singularity_message(ERROR, "Failed to enter a PID namespace: %s\n", strerror(errno));
            exit(1);
        }
        stage_pid = fork();
    } else {
    ...
    if (stage_pid == 0) {
        ...
        if (get_nspath(mnt) == NULL) {
            ...
            if (enter_namespace(get_nspath(mnt), CLONE_NEWNS) == -1) {
        ...
        if (get_nspath(mnt)) {
            if (config.isSuid && setresuid(uid, uid, uid) == -1) {
            ...
            if (config.isSuid && setresuid(uid, uid, 0) == -1) {
